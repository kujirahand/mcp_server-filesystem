...
```sh
uv run mcp-server-filesystem ~/Documents/work ~/tmp
```

モジュールとして起動することもできます。

```sh
uv run python -m mcp_server_filesystem ~/Documents/work
```
...
`uvx` を使うと、事前のインストールなしで実行できます(初回に自動でビルド・取得されます)。

ローカルのフォルダから実行する場合:

```sh
uvx --from /path/to/mcp_server-filesystem mcp-server-filesystem ~/Documents/work
```

GitHubから直接実行する場合:

```sh
uvx --from git+https://github.com/kujirahand/mcp_server-filesystem mcp-server-filesystem ~/Documents/work
```
...
```sh
claude mcp add filesystem -- uvx --from /path/to/mcp_server-filesystem mcp-server-filesystem ~/Documents/work
```
...
```sh
codex mcp add filesystem -- uvx --from /path/to/mcp_server-filesystem mcp-server-filesystem ~/Documents/work
```
...
```sh
npx -y @modelcontextprotocol/inspector uv run mcp-server-filesystem ~/Documents/work
```
...
`uvx` で登録した場合と同じ形で確認したいときは、`uv run` の部分を置き換えます。

```sh
npx -y @modelcontextprotocol/inspector uvx --from /path/to/mcp_server-filesystem mcp-server-filesystem ~/Documents/work
```
...
```sh
npx -y @modelcontextprotocol/inspector --cli uv run mcp-server-filesystem ~/Documents/work \
  --method tools/list
```
